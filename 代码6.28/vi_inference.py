...
#   "exp_matern_product" : data from exp_matern_product_simulation.py
...
#   "virus_spread" : data from virus_spread_simulation.py's virus propagation simulation
#   "saturation" : data from saturation_simulation.py's saturation propagation simulation
RUN_KINDS = ("exp_neighbor_sine",)
...
SIMULATION_DATA_FILES = {
    ...
    "exp_matern_product": os.path.join(
        SIMULATION_RESULTS_DIR,
        "exp_matern_product",
        "exp_matern_product_data.npz",
    ),
    ...
    "virus_spread": os.path.join(
        SIMULATION_RESULTS_DIR,
        "virus_spread",
        "virus_spread_data.npz",
    ),
    "saturation": os.path.join(
        SIMULATION_RESULTS_DIR,
        "saturation",
        "saturation_data.npz",
    ),
...
def save_tau_style_summary(save_dir, run_kind, guide_type, true_params, vi_params, metrics):
    ...
    summary_rows = []
    if true_params.get("data_generating_process") in {
        "tanh_nnngp",
        "mlp_nnngp",
        "matern_gp_nnngp",
    }:
...
def run_one_simulation(
    ...
    if not os.path.exists(data_path):
        generator_script = (
            "python NNnGP/saturation_simulation.py"
            if run_kind == "saturation"
            else "python NNnGP/virus_spread_simulation.py"
            if run_kind == "virus_spread"
...
def main():
    ...
    parser.add_argument(
        ...
        dest="optimize_z",
        action="store_false",
        help="Keep FIC inducing locations Z fixed. This is the default.",
    )
    parser.add_argument(
        "--optimize-z",
        dest="optimize_z",
