...
DEFAULT_DATA = EXPERIMENT_DIR / "data" / "no_split" / "exp_neighbor_sine_data.npz"
DEFAULT_SPLIT_DATA = EXPERIMENT_DIR / "data" / "split" / "exp_neighbor_sine_reference_split_data.npz"
DEFAULT_SAVE_DIR = EXPERIMENT_DIR / "outputs" / "no_split" / "nnngp"
DEFAULT_SPLIT_SAVE_DIR = EXPERIMENT_DIR / "outputs" / "split" / "nnngp"
DEFAULT_M_VALUES = tuple(range(4, 22, 2))
M_TILDE_OVERRIDES = {2: 10, 4: 20}


def ensure_simulation_data(data_path: Path) -> Path:
    """Generate a packaged sine dataset automatically when it is absent."""
    data_path = data_path.resolve()
    if data_path.exists():
        return data_path

    generators = {
        DEFAULT_DATA.resolve(): (
            Path(__file__).resolve().parent / "no_split_simulation" / "exp_neighbor_sine_simulation.py",
            "generate_exp_neighbor_sine_data",
        ),
        DEFAULT_SPLIT_DATA.resolve(): (
            Path(__file__).resolve().parent / "split_simulation" / "exp_neighbor_sine_split_simulation.py",
            "generate_reference_split_data",
        ),
    }
    if data_path not in generators:
        raise FileNotFoundError(
            f"Data file not found: {data_path}. Generate it first or provide an existing --data path."
        )

    module_path, function_name = generators[data_path]
    spec = importlib.util.spec_from_file_location(f"sine_generator_{function_name}", module_path)
    if spec is None or spec.loader is None:
        raise ImportError(f"Could not load simulation generator: {module_path}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    print(f"Simulation data missing; generating it at: {data_path}")
    getattr(module, function_name)(output_npz=data_path)
    if not data_path.exists():
        raise FileNotFoundError(f"Simulation generator did not create: {data_path}")
    return data_path
...
def make_order_specs(S: np.ndarray, order_count: int, seed: int) -> list[dict]:
    ...
    specs = [
        ...
        {"order_type": "x2_x1", "permutation": np.lexsort((S[:, 0], S[:, 1])).astype(int)},
    ]
...
def write_elbo_trace(path: Path, losses: np.ndarray) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        ...
        for idx, loss in enumerate(losses, start=1):
            writer.writerow([idx, f"{loss:.17g}"])


def print_elbo_trace(losses: np.ndarray, log_every: int, label: str) -> None:
    ...
    if len(losses) == 0:
        return
    print(f"\n[{label}] ELBO loss trace:")
    for idx in range(0, len(losses), log_every):
        print(f"  iter {idx + 1:6d}: loss={losses[idx]:.6g}")
    if (len(losses) - 1) % log_every != 0:
        print(f"  iter {len(losses):6d}: loss={losses[-1]:.6g}")
...
def run_combo(args, combo_index: int) -> dict:
    ...
    if finite_fraction(losses) < 1.0 or finite_fraction(vi_w_S) < 1.0 or finite_fraction(vi_Z) < 1.0:
        ...
        write_elbo_trace(elbo_path, losses)
        print_elbo_trace(losses, int(args.elbo_log_every), combo)
        row = {
            "combo": combo,
    ...
    np.savez_compressed(
        ...
        flows=np.asarray(args.flows, dtype=np.int32),
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="no_split",
        help="Packaged sine design to generate and fit when --data is omitted.",
    )
    parser.add_argument("--data", default=None, help="Optional custom NPZ path.")
    parser.add_argument("--save-dir", default=None, help="Optional custom output directory.")
    ...
    parser.add_argument(
        "--elbo-log-every",
        type=int,
        default=int(os.environ.get("VI_ELBO_LOG_EVERY", "0")),
        help="Print ELBO/loss trace every N iterations after each combo; 0 disables printing.",
    )
...
def main():
    args = parse_args()
    default_data = DEFAULT_DATA if args.dataset == "no_split" else DEFAULT_SPLIT_DATA
    default_save_dir = DEFAULT_SAVE_DIR if args.dataset == "no_split" else DEFAULT_SPLIT_SAVE_DIR
    args.data = ensure_simulation_data(Path(args.data) if args.data else default_data)
    args.save_dir = Path(args.save_dir).resolve() if args.save_dir else default_save_dir.resolve()
