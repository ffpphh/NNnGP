...
RESULTS_DIR = os.path.join(PACKAGE_ROOT, "outputs", "core", "data_utils_outputs")
...
def parametric_tanh_g(v, params=(1.0, 0.5, 0.0, 0.5)):
    """Signed tanh-style g(v).

    A 3-tuple keeps the legacy bounded form:
        amplitude * tanh(z)
    A 4-tuple uses the current unbounded form:
        amplitude * (z + gamma * tanh(z))
    where z = slope * mean(v) + bias.
    """
    if len(params) == 3:
        amplitude, slope, bias = params
        z = slope * np.mean(v) + bias
        return float(amplitude * np.tanh(z))
    if len(params) == 4:
        amplitude, slope, bias, gamma = params
        z = slope * np.mean(v) + bias
        return float(amplitude * (z + gamma * np.tanh(z)))
    raise ValueError("parametric_g_params must be a 3-tuple or 4-tuple.")
...
def generate_nnngp_data(
    ...
    parametric_g_params=(1.0, 0.5, 0.0, 0.5), # tanh g参数：(amplitude, slope, bias, gamma)
    ...
    if precompute_Z_with_NNGP:
        ...
        raw_g_samples = np.asarray([parametric_tanh_g(v, parametric_g_params) for v in v_samples], dtype=np.float64)
        g_mean = float(np.mean(raw_g_samples))
        g_std = float(np.std(raw_g_samples, ddof=0))
        if (not standardize_g) or g_std <= 1e-12:
            g_mean = 0.0
            g_std = 1.0
    ...
    for i in range(m, k):
        ...
        # 参数化非线性 g(v)，可正可负。
        g_i = (parametric_tanh_g(v_i, parametric_g_params) - g_mean) / g_std
    ...
    for i in range(len(U)):
        ...
        g_u = (parametric_tanh_g(v_u, parametric_g_params) - g_mean) / g_std
    ...
    np.savez_compressed(
        ...
        true_params={
            ...
            'g_type': 'parametric_tanh' if len(parametric_g_params) == 3 else 'parametric_linear_tanh',
            'g_standardized': bool(standardize_g),
            'g_standardization': (g_mean, g_std),
            'beta': beta,
    ...
    print(f"g_standardization=({g_mean:.6g}, {g_std:.6g}), standardize_g={standardize_g}")
    return save_path
...
def _simulate_fixed_s_state(S, Z, true_params, precomputed, rng):
    ...
    m = int(true_params["m"])
    k = len(S)
    parametric_g_params = true_params.get("parametric_g_params", (1.0, 0.5, 0.0, 0.5))
    g_mean, g_std = true_params.get("g_standardization", (0.0, 1.0))
    if not true_params.get("g_standardized", False):
...
def _simulate_w_at_locations(locations, S, w_S, Z, true_params, precomputed, g_Z, R_Z_inv, g_kernel, rng):
    ...
    for j, loc in enumerate(locations):
        ...
        if s_idx is not None:
            ...
            g_j = (parametric_tanh_g(v_j, parametric_g_params) - g_mean) / g_std
...
if __name__ == "__main__":
    ...
    parametric_g_params = (1.5, 1.5, 0.0, 0.5)
    # Four parameters use the unbounded tanh-style form:
    # g(v) = amplitude * (z + gamma * tanh(z)), z = slope * mean(v) + bias.
    # Three parameters keep the legacy bounded tanh form for old data/tests.
    beta = (0.5, 0.5, -0.5)
    sigma_epsilon = 0.1
