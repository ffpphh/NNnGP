...
# Kernels
# ==============================================================================
def rbf_kernel_jax(X, Y=None, length_scale=1.0):
    """JAX RBF correlation kernel for the latent nonlinear GP g."""
    X = jnp.asarray(X, dtype=jnp.float64)
    Y = X if Y is None else jnp.asarray(Y, dtype=jnp.float64)
    sq_dists = jnp.sum((X[:, None, :] - Y[None, :, :]) ** 2, axis=-1)
    return jnp.exp(-0.5 * sq_dists / (length_scale**2 + 1e-12))


def rbf_kernel_np(X, Y=None, length_scale=1.0):
    """NumPy RBF correlation kernel for prediction utilities."""
    X = np.asarray(X, dtype=np.float64)
    Y = X if Y is None else np.asarray(Y, dtype=np.float64)
    sq_dists = np.sum((X[:, None, :] - Y[None, :, :]) ** 2, axis=-1)
    return np.exp(-0.5 * sq_dists / (length_scale**2 + 1e-12))
...
def _fic_correction_from_reference_terms(W, tau, F, residual, Z, rho, R_Z, R_Z_chol):
    ...
    R_WZ = rbf_kernel_jax(W, Z, rho)  # (k-m, m_tilde)
    R_ZW = R_WZ.T
...
def compute_fic_correction(
    ...
    if R_Z is None:
        R_Z = rbf_kernel_jax(Z, Z, rho) + JITTER * jnp.eye(Z.shape[0], dtype=jnp.float64)
    else:
    ...
    return _fic_correction_from_reference_terms(W, tau, F, residual, Z, rho, R_Z, R_Z_chol)
...
def compute_eb_base_and_fic_correction_precomputed(
    ...
    correction = _fic_correction_from_reference_terms(W, tau, F, residual, Z, rho, R_Z, R_Z_chol)
...
def _reference_state_np(w_S, S, neighbors, m, Z, true_params):
    ...
    R_Z = rbf_kernel_np(Z, Z, rho) + JITTER * np.eye(len(Z))
    R_Z_inv = np.linalg.inv(R_Z)
    R_WZ = rbf_kernel_np(W, Z, rho)
    R_ZW = R_WZ.T
...
def _predict_one_location_np(w_S, geom_row, state, rng):
    ...
    R_vZ = rbf_kernel_np(v_j[None, :], state["Z"], state["rho"])[0]
...
def nnngp_model(
    ...
    if true_params is not None:
        ...
        rho = numpyro.sample("rho", dist.HalfNormal(1.0))
...
def nnngp_model_empirical_bayes(
    ...
    rho = numpyro.param(
        "eb_rho",
        jnp.asarray(_init_from_dict(init_params, "rho", 1.0), dtype=jnp.float64),
        constraint=dist.constraints.positive,
    )
...
# ==============================================================================
def _rbf_kernel_jax_fast(X, Y=None, length_scale=1.0):
    """RBF kernel that preserves the input dtype for faster GPU prediction."""
    X = jnp.asarray(X)
    Y = X if Y is None else jnp.asarray(Y, dtype=X.dtype)
    length_scale = jnp.asarray(length_scale, dtype=X.dtype)
    sq_dists = jnp.sum((X[:, None, :] - Y[None, :, :]) ** 2, axis=-1)
    return jnp.exp(-0.5 * sq_dists / (length_scale**2 + 1e-12))
...
def _predict_one_sample_all_locations_jax(
    ...
    R_WZ = _rbf_kernel_jax_fast(W, Z, rho)                     # (nW, mt)
    R_ZW = R_WZ.T                                             # (mt, nW)
    ...
    R_VZ = _rbf_kernel_jax_fast(V, Z, rho)                     # (r, mt)
    ...
    # R_{v|W} diagonal, with R(v,v)=1 for standardized RBF correlation.
...
def predict(
    ...
    R_Z_np = rbf_kernel_np(Z, Z, rho) + JITTER * np.eye(len(Z))
